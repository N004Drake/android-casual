...
public class TwrpCommunications {
    ...
    public void runTwrpScript(String script) throws IOException{
        Log.level2Information("verifying TWRP mode and rebooting if necessary");
        rebootTWRP();
        Log.level4Debug("writing recovery script to temp location");
        writeToFile(script,Statics.getTempFolder()+"openrecoveryscript");
        adb.run(new String[]{"push", Statics.getTempFolder()+"openrecoveryscript", "/cache/recovery/"}, timeout, false);
